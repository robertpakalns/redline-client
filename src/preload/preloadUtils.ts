...
// Search (incoming) friends
const filterFriends = (container: HTMLElement, search: string): void => {
  const query = search.toLowerCase().trim();
  const list =
    container.querySelector(".list") || container.querySelector(".requests");
  if (!list) return;
...
export const initFriendSearch = (container: HTMLElement): void => {
  const addFriends = container.querySelector(".add-friends");
  if (!addFriends) return;

  let input = addFriends.querySelector<HTMLInputElement>(".friendFilter input");
  ...
  if (!input) {
    ...
    input.addEventListener("input", () =>
      filterFriends(container, input!.value),
    );

    const friendSearchCont = createEl("div", {}, "friendFilter", [
      label,
      input,
    ]);
    addFriends.appendChild(friendSearchCont);
  }

  // if (!(input as any)._listenerAdded) {
  //   input.addEventListener("input", () =>
  //     filterFriends(container, input!.value),
  //   );
  //   (input as any)._listenerAdded = true;
  // }
};
